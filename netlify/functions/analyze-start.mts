...
function jobStore(){
  return getStore("avantoffre-jobs", { consistency: "strong" });
}
...
export default async(req:Request,_context:Context)=>{
  ...
  try{
    ...
    let body:any;
    try{body=await req.json()}catch{return json({error:"La demande envoyée à AvantOffre est invalide."},400)}
    ...
    if(!listingUrl&&!address&&documents.length===0)return json({error:"Ajoutez au moins une annonce, une adresse ou un document."},400);

    await store.setJSON(`input-${jobId}`,{listingUrl,address,documents,extra});
