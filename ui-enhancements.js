...
.ao-docstatus{margin-left:auto;margin-right:10px;display:flex;align-items:center;gap:7px;min-width:175px;max-width:280px;font-size:11px;font-weight:800}.ao-dot{width:21px;height:21px;border-radius:50%;display:grid;place-items:center;font-size:12px;flex:0 0 21px}.ao-docstatus.ok{color:#0a806b}.ao-docstatus.ok .ao-dot{background:#13a584;color:#fff}.ao-docstatus.wait{color:#71818c;font-weight:700}.ao-docstatus.wait .ao-dot{background:#edf2f4;color:#71818c}.ao-docstatus.bad{color:#c74646}.ao-docstatus.bad .ao-dot{background:#e95757;color:#fff}.ao-docstatus.warn{color:#b36b00}.ao-docstatus.warn .ao-dot{background:#f2a11a;color:#fff}.ao-docstatus.partial{color:#9a6500}.ao-docstatus.partial .ao-dot{background:#f1a51b;color:#fff}.ao-docstatus.graphical{color:#3156a4}.ao-docstatus.graphical .ao-dot{background:#e8efff;color:#1258ff}.ao-docreason{display:block;font-size:9.5px;font-weight:500;color:#687b89;margin-top:2px;line-height:1.25}.ao-readalert{margin:11px 0;padding:11px 13px;border:1px solid #efc3c3;border-radius:13px;background:#fff7f7;color:#713d3d;font-size:10.5px;line-height:1.4}.ao-readalert strong{display:block;color:#8e3737;font-size:11.5px;margin-bottom:4px}.ao-readalert ul{margin:5px 0 0;padding-left:17px}.ao-readalert li{margin:4px 0}.ao-readalert .ao-solution{display:block;margin-top:2px;color:#526a7b}.ao-readalert.critical{border-width:2px}.ao-readalert.report{margin-bottom:14px}.ao-readalert .ao-file{font-weight:850;color:#5d3030}.ao-readalert.summary{border-color:#dce6eb;background:#f8fafb;color:#526a7b}.ao-readalert.summary strong{color:#17384f}.ao-readalert.summary.has-issue{border-color:#efd7ba;background:#fffaf3}.ao-readalert.summary.has-issue strong{color:#7d5416}.ao-readalert.summary .ao-summaryline{margin-top:5px}.ao-readalert.summary .ao-summaryissue{margin-top:7px;padding-top:7px;border-top:1px solid #ece3d6}.ao-readalert.summary .ao-summaryissue .ao-file{color:#714c1e}
...
const readState=new Map();const isPv=name=>/\b(pv|ag)\b|assembl/i.test(name||'');const isGraphicName=name=>/(?:^|[_\s.-])(?:plan|plan_apt|plan-appartement|plan appartement)(?:[_\s.-]|$)/i.test(name||'');function rows(){return [...document.querySelectorAll('#filesList .file')]}function fileNamesFromRows(){return rows().map(r=>r.querySelector('.fileName')?.textContent||'document')}
...
function setRowStatus(i,state){
  const row=rows()[i];if(!row)return;let box=row.querySelector('.ao-docstatus');
  if(!box){box=document.createElement('div');box.className='ao-docstatus';const remove=row.querySelector('button');remove?row.insertBefore(box,remove):row.appendChild(box)}
  if(!state){box.className='ao-docstatus wait';box.innerHTML='<span class="ao-dot">·</span><span>En attente</span>';return}
  if(state.status==='ok'){box.className='ao-docstatus ok';box.innerHTML='<span class="ao-dot">✓</span><span>Traité</span>';return}
  if(state.status==='partial'){box.className='ao-docstatus partial';box.innerHTML='<span class="ao-dot">!</span><span>Exploitable<small class="ao-docreason">Certaines pages sont moins lisibles.</small></span>';return}
  if(state.status==='graphical'){box.className='ao-docstatus graphical';box.innerHTML='<span class="ao-dot">▧</span><span>Plan détecté<small class="ao-docreason">Texte lisible exploité ; géométrie à confirmer.</small></span>';return}
  box.className='ao-docstatus bad';box.innerHTML=`<span class="ao-dot">×</span><span>À vérifier<small class="ao-docreason">${esc(state.solution||state.reason||'Lecture insuffisante.')}</small></span>`;
}
function renderFileStates(){rows().forEach((_,i)=>setRowStatus(i,readState.get(i)))}
window.addEventListener('revisite-document-issue',e=>{
  ...
  readState.set(i,{name,status:'failed',reason:isGraphicName(name)?'Plan à vérifier':'Document à vérifier',solution:isGraphicName(name)?'ReVisite a tenté une lecture OCR, mais le texte détecté reste insuffisant pour exploiter ce plan automatiquement.':'ReVisite n’a pas obtenu assez de contenu fiable. Réimportez une version plus nette.',kind:'bad',chars:0});
...
function installUploadMonitor(){
  ...
  window.fetch=async(input,init)=>{
    ...
    if(url.includes('/api/upload-document')){
      try{const b=JSON.parse(String(init?.body||'{}'));const docs=Array.isArray(b?.documents)?b.documents:[b];metas=docs.map(x=>({name:String(x?.name||'document'),index:Number(x?.index),documentKind:String(x?.documentKind||'')})).filter(x=>Number.isFinite(x.index))}catch{}
    }
    try{
      ...
      if(metas.length){
        ...
        for(const meta of metas){
          ...
          const documentKind=String(item?.documentKind||meta.documentKind||'');
          if(res.ok&&documentKind==='graphical'&&item?.accepted!==false)readState.set(meta.index,{name:meta.name,status:'graphical',reason:'Plan détecté',solution:'Le texte réellement détecté est exploité ; aucune dimension ou géométrie n’est déduite si elle n’est pas lisible.',kind:'info',chars:actualChars});
          else if(res.ok&&quality==='partial'&&item?.accepted!==false)readState.set(meta.index,{name:meta.name,status:'partial',reason:'Lecture partielle',solution:'Le document reste exploitable ; certaines pages sont moins lisibles.',kind:'warn',chars:actualChars});
          else if(res.ok&&item?.accepted!==false&&actualChars>=80)readState.set(meta.index,{name:meta.name,status:'ok',chars:actualChars});
          else if(documentKind==='graphical'||isGraphicName(meta.name))readState.set(meta.index,{name:meta.name,status:'failed',reason:'Plan à vérifier',solution:'ReVisite a tenté une seconde lecture OCR, mais le texte détecté reste insuffisant pour exploiter ce plan automatiquement.',kind:'bad',chars:actualChars});
...
function failures(includeMissing=false){
  const names=fileNamesFromRows(),out=[];
  names.forEach((name,i)=>{const s=readState.get(i);if(s?.status==='failed')out.push({...s,index:i});else if(includeMissing&&!s)out.push({name,index:i,status:'failed',reason:'Fichier non traité',solution:'Réimportez le document puis relancez l’analyse.',kind:'bad'})});
  return out
}
function renderReadWarnings(final=false){
  const names=fileNamesFromRows(),states=names.map((name,i)=>readState.get(i)||null),pending=states.filter(x=>!x).length;
  const side=document.querySelector('.sideCard');
  if(side){
    let el=document.querySelector('#aoReadWarning');
    if(!names.length||(!final&&pending>0)){el?.remove()}
    else{
      const hard=failures(final),partial=states.filter(x=>x?.status==='partial'),graphical=states.filter(x=>x?.status==='graphical');
      const usable=states.filter(x=>x&&['ok','partial','graphical'].includes(x.status)).length,total=names.length;
      const lines=[];
      if(partial.length)lines.push(`<div class="ao-summaryline">${partial.length} document${partial.length>1?'s':''} comporte${partial.length>1?'nt':''} quelques pages moins lisibles, sans empêcher l’analyse.</div>`);
      if(graphical.length)lines.push(`<div class="ao-summaryline">${graphical.length} document${graphical.length>1?'s graphiques détectés':' graphique détecté'} : le texte lisible est exploité, sans inventer les dimensions ou la géométrie.</div>`);
      if(hard.length){
        const items=hard.map(x=>`<li><span class="ao-file">${esc(x.name)}</span> — ${esc(x.reason||'Lecture insuffisante')}<span class="ao-solution">${esc(x.solution||'Réimportez une version plus nette.')}</span></li>`).join('');
        lines.push(`<div class="ao-summaryissue"><b>${hard.length} document${hard.length>1?'s nécessitent':' nécessite'} une vérification :</b><ul>${items}</ul></div>`);
      }
      const html=`<div class="ao-readalert summary ${hard.length?'has-issue':''}"><strong>${usable} document${usable>1?'s':''} exploitable${usable>1?'s':''} sur ${total}</strong>${lines.join('')}</div>`;
